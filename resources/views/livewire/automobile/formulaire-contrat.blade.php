        ...
        <form wire:submit.prevent="save" class="space-y-6">
            ...
            @php
                $isAdmin = auth()->check() && (
                    ...
                    !empty(auth()->user()->is_super_admin)
                );
            @endphp
            ...
            @else
                <!-- Non-Admin Staff View: Simplified Totals Only (Margins Hidden) -->
                <div class="bg-slate-900 text-white rounded-2xl p-5 shadow-lg space-y-4">
                    <div class="flex items-center justify-between border-b border-slate-800 pb-2">
                        <div class="flex items-center gap-2">
                            <span class="h-2.5 w-2.5 rounded-full bg-teal-400"></span>
                            <h3 class="text-xs font-black uppercase tracking-wider text-slate-200">Récapitulatif du Tarif Contrat</h3>
                        </div>
                    </div>
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4 text-center divide-y md:divide-y-0 md:divide-x divide-slate-800">
                        <div class="py-1">
                            <span class="text-[11px] font-bold uppercase tracking-wider text-slate-400 block">Prime Nette (HT)</span>
                            <div class="text-xl font-bold text-white mt-1 font-mono">{{ number_format($this->primeNette, 2) }} DH</div>
                        </div>
                        <div class="py-1">
                            <span class="text-[11px] font-bold uppercase tracking-wider text-slate-400 block">Taxes & Accessoires</span>
                            <div class="text-xl font-bold text-slate-300 mt-1 font-mono">{{ number_format($this->totalTaxe, 2) }} DH</div>
                        </div>
                        <div class="py-1">
                            <span class="text-[11px] font-bold uppercase tracking-wider text-teal-400 block">Prime Totale (TTC)</span>
                            <div class="text-2xl font-extrabold text-teal-400 mt-1 font-mono">{{ number_format($this->primeTotale, 2) }} DH</div>
                        </div>
                    </div>
                </div>
            @endif
