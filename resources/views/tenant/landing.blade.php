...
<head>
    ...
    <meta name="description" content="Agence d'assurance agréée et conseil en courtage au Maroc. Solutions Automobile, Habitation, Santé, Entreprise.">
    ...
    <link href="https://fonts.googleapis.com/css2?family=Plus+Jakarta+Sans:wght@400;500;600;700;800;900&family=Noto+Kufi+Arabic:wght@400;600;700;900&display=swap" rel="stylesheet">
...
<body class="min-h-screen selection:bg-indigo-500 selection:text-white overflow-x-hidden bg-white text-slate-900">
    ...
    <!-- Top Info Bar (Enterprise Standard) -->
        ...
        <div class="max-w-7xl mx-auto px-6 h-20 flex items-center justify-between">
            <div class="flex items-center gap-3.5">
                ...
                <div>
                    ...
                    <span class="block text-[9.5px] font-bold uppercase tracking-wider text-slate-500 -mt-0.5">
                        <span x-show="lang === 'fr'">Agence Agréée d'Assurance & Courtage</span>
            ...
            <div class="flex items-center gap-8">
                ...
                <nav class="hidden lg:flex items-center gap-6 text-xs font-semibold text-slate-600">
                    <a href="#services" class="hover:text-slate-900 transition">
                        <span x-show="lang === 'fr'">Nos Offres</span>
                        <span x-show="lang === 'ar'" style="display:none;">عروضنا</span>
                    </a>
                    <a href="#sinistres" class="hover:text-slate-900 transition">
                        <span x-show="lang === 'fr'">Déclaration Sinistre</span>
                        <span x-show="lang === 'ar'" style="display:none;">التصريح بالحادث</span>
                    </a>
                    <a href="#agencies" class="hover:text-slate-900 transition">
                        <span x-show="lang === 'fr'">Nos Agences</span>
                        <span x-show="lang === 'ar'" style="display:none;">وكالاتنا</span>
                    </a>
                ...
                <button @click="quoteModal = true" class="text-white font-bold text-xs px-5 py-2.5 {{ $radius }} shadow-xs transition flex items-center gap-2 hover:opacity-95" style="background-color: {{ $primaryColor }}">
                    ...
                    <span x-show="lang === 'fr'">Obtenir Devis</span>
    ...
    <!-- Hero Section (Light Enterprise Standard) -->
    <section class="bg-gradient-to-b from-slate-50 to-white py-20 border-b border-slate-200/80">
        <div class="max-w-7xl mx-auto px-6 grid md:grid-cols-12 gap-12 items-center">
            <div class="md:col-span-7 space-y-6">
                <div class="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-slate-100 border border-slate-200 text-xs font-semibold text-slate-700">
                    ...
                    <span x-show="lang === 'fr'">Conseil & Courtage d'Assurance Habilité au Maroc</span>
                    <span x-show="lang === 'ar'" style="display:none;">استشارات وتأمين معتمد بالمملكة المغربية</span>
                </div>

                <h1 class="text-3xl md:text-5xl font-extrabold text-slate-900 tracking-tight leading-tight">
                    <span x-show="lang === 'fr'">Protégez votre famille, vos biens & votre entreprise avec <span style="color: {{ $primaryColor }}">{{ $agencyName }}</span></span>
                    <span x-show="lang === 'ar'" style="display:none;">احمِ عائلتك وممتلكاتك وشركتك مع <span style="color: {{ $primaryColor }}">{{ $agencyName }}</span></span>
                </h1>

                <p class="text-slate-600 text-sm md:text-base max-w-xl leading-relaxed font-medium">
                    <span x-show="lang === 'fr'">Votre interlocuteur de confiance au Maroc. Solutions d'assurances complètes pour Automobile, Multirisque Habitation, Santé et Flottes Entreprises avec garanties validées sous 2h.</span>
                    <span x-show="lang === 'ar'" style="display:none;">وكالتكم الموثوقة بالمغرب. حلول تأمينية شاملة للسيارات، المنازل، الصحة والشركات مع التزام تام بسرعة التجاوب وتقديم أفضل العروض.</span>
                </p>
                ...
                <div class="flex flex-col sm:flex-row items-stretch sm:items-center gap-4 pt-2">
                    <button @click="quoteModal = true" class="text-white font-bold text-xs uppercase tracking-wider px-8 py-3.5 {{ $radius }} shadow-xs transition flex items-center justify-center gap-2" style="background-color: {{ $primaryColor }}">
                        <span x-show="lang === 'fr'">Demander Un Devis En Ligne</span>
                        <span x-show="lang === 'ar'" style="display:none;">طلب تسعيرة عبر الإنترنت</span>
                    </button>
            ...
            <div class="md:col-span-5">
                <div class="bg-white border border-slate-200 rounded-2xl p-8 space-y-6 shadow-md">
                    <div class="flex items-center gap-3 border-b border-slate-100 pb-4">
                        ...
                        <div>
                            <h3 class="font-bold text-base text-slate-900">
                                <span x-show="lang === 'fr'">Devis Rapide sous 2 heures</span>
                                <span x-show="lang === 'ar'" style="display:none;">تسعيرة سريعة خلال ساعتين</span>
                            </h3>
                            <span class="text-xs text-slate-500 font-medium">Garantie des meilleurs tarifs compagnies</span>
                        </div>
                    </div>
                    ...
                    <div class="space-y-3 text-xs">
                        ...
                        <div class="flex items-center justify-between p-3 bg-slate-50 rounded-xl border border-slate-100 font-semibold">
                            <span class="text-slate-600">Multirisque Habitation & Dégâts</span>
                            <span class="text-indigo-600">Disponible</span>
                        </div>
                        <div class="flex items-center justify-between p-3 bg-slate-50 rounded-xl border border-slate-100 font-semibold">
    ...
    <section class="py-10 bg-white border-b border-slate-200">
        <div class="max-w-7xl mx-auto px-6 text-center space-y-4">
            ...
            <div class="flex flex-wrap justify-center items-center gap-10 md:gap-16 font-extrabold text-slate-400 text-sm tracking-tight">
                <span class="hover:text-slate-800 transition cursor-pointer">AXA ASSURANCE</span>
                <span class="hover:text-slate-800 transition cursor-pointer">RMA WATANIYA</span>
                <span class="hover:text-slate-800 transition cursor-pointer">SANLAM MAROC</span>
                <span class="hover:text-slate-800 transition cursor-pointer">WAFA ASSURANCE</span>
                <span class="hover:text-slate-800 transition cursor-pointer">ZURICH</span>
                <span class="hover:text-slate-800 transition cursor-pointer">ALLIANZ</span>
            </div>
        </div>
    </section>

    <!-- 12 Insurance Categories & Services Grid -->
    <section id="services" class="py-20 bg-[#F8FAFC]">
        <div class="max-w-7xl mx-auto px-6 space-y-12">
            <div class="text-center space-y-2 max-w-2xl mx-auto">
                <span class="text-[10px] font-bold uppercase tracking-wider text-indigo-600">Solutions Complètes</span>
                <h2 class="text-3xl font-extrabold text-slate-900 tracking-tight">Nos Produits d'Assurances</h2>
                <p class="text-xs text-slate-500 font-medium">Une gamme d'offres adaptées aux besoins des particuliers, professionnels et grandes entreprises.</p>
            </div>
            ...
            <div class="grid md:grid-cols-3 gap-6">
                ...
                <div class="bg-white p-6 rounded-2xl border border-slate-200 shadow-xs hover:shadow-md transition space-y-4">
                    <div class="w-12 h-12 rounded-xl bg-purple-50 border border-purple-100 flex items-center justify-center text-purple-600">
                        <svg class="w-6 h-6" fill="none" stroke="currentColor" stroke-width="1.75" viewBox="0 0 24 24"><rect width="16" height="20" x="4" y="2" rx="2" ry="2"/><line x1="9" x2="9.01" y1="6" y2="6"/><line x1="15" x2="15.01" y1="6" y2="6"/><line x1="9" x2="9.01" y1="10" y2="10"/><line x1="15" x2="15.01" y1="10" y2="10"/><line x1="9" x2="9.01" y1="14" y2="14"/><line x1="15" x2="15.01" y1="14" y2="14"/><path d="M9 18h6"/></svg>
                    </div>
                    <h3 class="font-bold text-base text-slate-900">Risques Entreprises</h3>
                    <p class="text-xs text-slate-500 leading-relaxed font-medium">Accident du travail (AT), responsabilité civile professionnelle (RC) et pertes d'exploitation.</p>
                </div>

                <!-- 5. Voyage -->
                <div class="bg-white p-6 rounded-2xl border border-slate-200 shadow-xs hover:shadow-md transition space-y-4">
                    <div class="w-12 h-12 rounded-xl bg-amber-50 border border-amber-100 flex items-center justify-center text-amber-600">
                        <svg class="w-6 h-6" fill="none" stroke="currentColor" stroke-width="1.75" viewBox="0 0 24 24"><path d="M17.8 19.2 16 11l3.5-3.5C21 6 21.5 4 21 3c-1-.5-3 0-4.5 1.5L13 8 4.8 6.2c-.5-.1-.9.1-1.1.5l-.3.5c-.2.5-.1 1 .3 1.3L9 12l-2 3H4l-1 1 3 2 2 3 1-1v-3l3-2 3.7 5.2c.3.4.8.5 1.3.3l.5-.3c.4-.2.6-.6.5-1.1z"/></svg>
                    </div>
                    <h3 class="font-bold text-base text-slate-900">Assurance Voyage Schengen</h3>
                    <p class="text-xs text-slate-500 leading-relaxed font-medium">Attestation médicale officielle et assistance rapatriement conforme visa Schengen.</p>
                </div>

                <!-- 6. Flottes -->
                <div class="bg-white p-6 rounded-2xl border border-slate-200 shadow-xs hover:shadow-md transition space-y-4">
                    <div class="w-12 h-12 rounded-xl bg-sky-50 border border-sky-100 flex items-center justify-center text-sky-600">
                        <svg class="w-6 h-6" fill="none" stroke="currentColor" stroke-width="1.75" viewBox="0 0 24 24"><path d="M16 16v1a2 2 0 0 1-2 2H3a2 2 0 0 1-2-2V7a2 2 0 0 1 2-2h11a2 2 0 0 1 2 2v1"/><path d="M18 8h4a2 2 0 0 1 2 2v7a2 2 0 0 1-2 2h-4"/><circle cx="5.5" cy="18.5" r="2.5"/><circle cx="18.5" cy="18.5" r="2.5"/></svg>
                    </div>
                    <h3 class="font-bold text-base text-slate-900">Gestion de Flottes Auto</h3>
                    <p class="text-xs text-slate-500 leading-relaxed font-medium">Optimisation des primes et couverture globale pour parcs automobiles de sociétés.</p>
                </div>
            </div>
        </div>
    </section>
    ...
    <section class="py-16 bg-white border-y border-slate-200">
        <div class="max-w-7xl mx-auto px-6 grid grid-cols-2 md:grid-cols-4 gap-8 text-center">
            <div class="space-y-1">
                <div class="text-3xl font-extrabold text-slate-900 tracking-tight">15 000+</div>
                <div class="text-xs font-semibold text-slate-500">Clients Assurés au Maroc</div>
            </div>
            <div class="space-y-1">
                <div class="text-3xl font-extrabold text-slate-900 tracking-tight">98%</div>
                <div class="text-xs font-semibold text-slate-500">Taux de Satisfaction</div>
            </div>
            <div class="space-y-1">
                <div class="text-3xl font-extrabold text-slate-900 tracking-tight">&lt; 2 Heures</div>
                <div class="text-xs font-semibold text-slate-500">Délai d'Étude Devis</div>
            </div>
            <div class="space-y-1">
                <div class="text-3xl font-extrabold text-slate-900 tracking-tight">24h / 7j</div>
                <div class="text-xs font-semibold text-slate-500">Assistance Dépannage</div>
            </div>
        </div>
    </section>
    ...
    <footer id="contact" class="bg-slate-900 text-slate-400 py-12 text-xs font-medium border-t border-slate-800">
        <div class="max-w-7xl mx-auto px-6 flex flex-col md:flex-row justify-between items-center gap-6">
            ...
            <div class="text-right text-slate-500">
                ...
                <span class="text-[10px] text-indigo-400 font-semibold block mt-0.5">Propulsé par Insurio Enterprise Website Builder (v3.0)</span>
            </div>
        </div>
    </footer>
